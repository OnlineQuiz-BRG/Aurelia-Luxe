...
const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView }) => {
  const { toggleWishlist, currentUser } = useStore();
  ...
  return (
    <div className="group relative bg-white border border-transparent hover:border-gold/20 transition-all duration-500 overflow-hidden shadow-sm hover:shadow-xl">
      {/* Label */}
      {product.isNew && (
        <span className="absolute top-4 left-4 z-10 bg-gold text-white text-[10px] uppercase tracking-widest px-3 py-1">
          New
        </span>
      )}
      ...
      <div className="p-6 text-center">
        ...
        <p className="text-xs font-light text-charcoal/80 mb-4 italic">From ${product.basePrice.toLocaleString()}</p>
        <button 
          onClick={() => onQuickView(product)}
